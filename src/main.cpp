// Pounce! — a single-player 2D platformer I built for my OOP course.
//
// It runs on OpenGL with GLUT driving the window and input loop, and SOIL
// loading the PNG artwork. The whole game is a small state machine in the Game
// class: a `menu` value selects the current screen (main menu, level select,
// death screen) and a `level` value selects the active level. Everything is
// drawn in immediate-mode OpenGL and stepped on a 33 ms GLUT timer.
//
// Controls: the arrow keys move and jump the ball, Esc quits. Collect every
// coin and, after grabbing the super-coin, eat the enemies to open the
// next-level door.
//
// Run the game from the project root so the relative "assets/..." texture
// paths resolve.
...
GLuint loadtexture(const char* name) {
    ...
    temp =
        SOIL_load_OGL_texture(name, SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_DDS_LOAD_DIRECT);

    return temp;
}

// Draws the currently bound texture as a full-screen quad. Every menu and level
// uses this to paint its background image before the game objects go on top.
void background() {
...
// The player's bouncing ball, and the base for the enemies and the boss, which
// reuse its position, radius and drawing. `life` flips to 0 when the ball dies,
// which the Game uses to switch to the death screen.
class Ball {
  protected:
    float Xco;
    float Yco;
...
class movingPlatform : public Platform {
  ...
  public:
    movingPlatform(float a, float b, float c, float d, float begin, float end)
        : Platform(a, b, c, d), beginPoint(begin), endPoint(end) {}
...
bool Coin::Collision(Ball a) {
    if (((a.surfaceS() + a.Radius() > Xco)) && (a.surfaceS() + a.Radius() < (Xco + 2 * rad)) &&
        (a.height() > (Yco - rad)) && (a.height() < (Yco + rad))) {
        return true;
    } else
        return false;
}
...
bool Door::Collision(Ball ball) {
    if (ball.surfaceS() > Xco && ball.surfaceS() < Xco + width && ball.height() > Yco &&
        ball.height() < Yco + height)
        return true;
    else
        return false;
}
...
void superCoin::Collision(Ball& ball) {
    ...
    if (((Xco > ball.surfaceS()) && (Xco < (ball.surfaceS() + 2 * ball.Radius())) &&
         (Yco > ball.height()) && (Yco < (ball.height() + 2 * ball.Radius())))) {
        vanish = 1;
    }
}
...
// Two different paths send the player into level 2 (the level-select menu and the
// level-1 exit door), so I keep the level-2 platform/coin/door layout in one place.
...
// The state machine that ties everything together. `menu` chooses the active
// screen (1 = main menu, 2 = level select, 3 = death, 0 = in a level) and
// `level` chooses which level to render. playGame() runs once per frame and
// branches on that pair to draw and update the right screen.
class Game {
    int menu;
    int level;
...
void Game::playGame() {
    if (menu == 1 && level == 0) {
        _mainMenuTextureId = loadtexture("assets/Game Menu Main.png");
        background();
    ...
    if (menu == 2 && level == 0) {
        _mainMenuTextureId = loadtexture("assets/Levels.png");
        background();
    ...
    if (menu == 3 && level == 0) {
        _mainMenuTextureId = loadtexture("assets/Dead.png");
        background();
    ...
    if (menu == 0 && level == 1) {
        _mainMenuTextureId = loadtexture("assets/Volcano.png");
        background();
        door1.drawDoor();
        door2.drawDoor();
    ...
    if (menu == 0 && level == 2) {
        _mainMenuTextureId = loadtexture("assets/Level2.png");
        background();
        door1.drawDoor();
        door2.drawDoor();
        if (coins.Van() == 1 && coins1.Van() == 1 && coins2.Van() == 1 && coins3.Van() == 1 &&
...
// Sets up the GLUT window, wires the input/draw/timer callbacks, and hands
// control to GLUT's event loop, which runs the game until the window closes.
int main(int argc, char** argv) {
    glutInit(&argc, argv);

    glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGBA); // double-buffered RGBA
    ...
    glutTimerFunc(33, myTimerFunction, 0); // ~30 FPS step
...
void init(void) {
    glClearColor(0.0, 0.0, 0.0, 0.0); // clear to black each frame

    // Use a 2D orthographic projection so I can position everything in pixel
    // coordinates instead of dealing with a 3D camera.
